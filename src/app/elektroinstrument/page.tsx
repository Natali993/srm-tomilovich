...
export default function Elektroinstrument() {
  const { generateHref } = useCollectionHref();
  const elektroinstrumentItems = collections.filter(item => item.type === TOOL_TYPE.ELECTRO);

  return (
    <CustomLayout breadcrumbs={()=><Breadcrumbs title="Електроінструмент" />} >
